...
# 深 Ta 角落的 depth 覆盖（成分判据）。默认 depth=0.95 对绝大多数合金有效，但富 Ta
# 合金在 depth>=0.90 时压缩端 SCF 第 4 步就崩。全库统计确认这是一个近乎二值的转变：
# Ta<60% 压缩端失败率 0~3%，Ta>=70% 骤升到 67~100%。故对 Ta>=DEEP_TA_THRESHOLD 的
# 合金改用 DEEP_TA_DEPTH。depth 扫描（DFT_0035 两端）显示阈值陡峭：<=0.85 收敛且能量
# 一致（亚 mRy），>=0.90 全失败 —— 取 0.80 给足悬崖余量。
#
# 注意：depth 会系统性微移能量，同一次 EOS 拟合的所有点必须用同一 depth。判据在
# 输入生成时应用（emto_generator._params_for），所以每个 stage 各自内部一致；stage1
# 里 Ta>=70% 但当初在 0.95 下拟合的合金不重算，其 sws0 仅作 stage2 的中心，stage2
# 会在 0.80 下重新精细拟合、自然把压缩端收敛回来。
DEEP_TA_THRESHOLD = 70.0   # Ta 原子百分比，>= 此值用 DEEP_TA_DEPTH
DEEP_TA_DEPTH = 0.80
