...
用法: jobs/submit_stage.sh <1|2|3> [选项]
  ...
  --only DFT_ID    只提交指定合金（可重复，用于冒烟测试）
...
### 冒烟测试：DFT_0001 的 6 个点

```bash
jobs/submit_stage.sh 1 --only DFT_0001 --chunk 6 --time 00:30:00 --partition deflt_short
```

验证 6 件事：

1. module 环境在计算节点上与登录节点一致，`kgrn_cpa` 动态库全部解析
2. `run_one.sh` 的 `cd` 使输出落在 `stage1_eos_coarse/DFT_0001/kgrn/`，而非仓库根
3. `.kgrn` 中 `FOR001=.../structures/kstr/bcc.tfh` 等绝对路径在节点上可读
4. **单点真实墙钟时间** → 回头校准 `--chunk` 与 `EMTO_TIMEOUT`
5. 幂等性：再次执行同一命令，worklist 应为空（6 个点全部被跳过）
6. `python run_pipeline.py --stage 1 --analyze` 能消费该输出并拟合出 SWS0/B0

### 参数校准与全量提交

冒烟测试通过并取得实测耗时后，重定 `--chunk` / `EMTO_TIMEOUT`，再全量提交
stage 1。stage 2、3 沿用校准后的参数（stage 2 单点应更快，因为初值更接近平衡）。
...
jobs/submit_stage.sh 1 --only DFT_0001 --chunk 6 --time 00:30:00 -p deflt_short  # 冒烟
