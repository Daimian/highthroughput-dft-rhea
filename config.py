...
# 逐合金 depth 覆盖。默认 depth=0.95 对绝大多数合金有效，但最深的 Ta 角落
# （Ta 81~90%）在 depth>=0.90 时 SCF 第 4 步就崩、depth<=0.85 才收敛。depth 扫描
# （DFT_0035 两端）显示阈值陡峭：0.80/0.85 收敛且能量一致（亚 mRy），0.90 起全失败。
# 取 0.80 给足悬崖余量。注意：depth 会系统性微移能量，同一合金所有点必须用同一
# depth（这里覆盖的合金要整体在 0.80 下重算，不能与旧的 0.95 点混拟合）。
DEPTH_OVERRIDES = {aid: 0.80 for aid in (
    'DFT_0035', 'DFT_0192', 'DFT_0193', 'DFT_0194',
    'DFT_0211', 'DFT_0213', 'DFT_0244',
)}
